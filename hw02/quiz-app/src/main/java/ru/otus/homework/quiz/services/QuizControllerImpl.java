...
public class QuizControllerImpl implements QuizController {
	...
	public QuizControllerImpl(IOService ioService, StudentService studentService, QuizFormatterImpl quizFormatter) {
	...
	private <T> T request(String prompt, ReplyParser<T> replyParser) {
		...
		try {
			...
			var errorMessage = quizFormatter.getErrorMessage(ex);
	...
	private Set<String> askQuestion(Question question) {
		...
		case OPTION_ID:
			return request(prompt, t -> (Set<String>) checkChooseOneOption(t, question.availableOptions()));
		case OPTION_SET:
			return request(prompt, t -> checkSelectOptionIds(t, question.availableOptions()));
		default:
			// impossible
			throw new IllegalArgumentException("Unexpected value: " + question.answerType());
		}
	}
