...
class BookControllerTest {
    ...
    @DisplayName("Попытка показать страницу удаления книги без авторизации")
    @Test
    void deletePageWithoutAuthority() throws Exception {
    ...
    void listPage() throws Exception {
        ...
        given(bookService.listBooks()).willReturn(books);
    ...
    void saveBook() throws Exception {
        ...
        mvc.perform(post("/book/1/edit")
                        ...
                        .param("ageLimit", Integer.toString(book.getAgeLimit()))
                )
    ...
    void addBook() throws Exception {
        val book = createBookDto(1L);
        given(bookService.createBook(any(), any(), any(), any()))
    ...
    private static BookProjection createBookProjection(long id) {
        ...
        return new BookProjection(id, "book" + id, author, genre, 6, 0);
    }

    private static BookDto createBookDto(long id) {
        val author = new AuthorDto(id, "author" + id);
        val genre = new GenreDto(id, "genre" + id);
        return new BookDto(id, "book" + id, author, genre, 6, null);
    }
    ...
    private static BookProjection toBookProjection(BookDto book) {
        ...
        return new BookProjection(book.getId(), book.getTitle(), book.getAuthor(),
                book.getGenre(), book.getAgeLimit(), numberOfComments);
    }
}